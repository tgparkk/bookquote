...
**상태**: Stage 0~1 완료 + 화면 설계(Stage 0b 연장) 완료 + Stage 2 본 작업 종료(PR1~6 + 책 별점) + **Stage 3 진행 중 — PR7(5종 템플릿 위젯 + 디스패처 + 에디터 mock 미리보기) 완료. 다음은 PR8(팔레트 서비스)**. `flutter analyze` clean, `flutter test` 59개 통과. 마이그레이션 4개(`quotes`, `user_books.rating`, `my_quote_mood_counts`, +Stage1 4개) 원격 적용 완료. main에 push됨. 릴리스 APK 빌드해 실기기(SM F956N) 설치 진행.
...
**바로 이어서 할 것** → **Stage 3 PR8 (팔레트 서비스)**. `palette_service.dart`(LRU 메모리 캐시) + `extractPalette(coverUrl)` (`palette_generator` `PaletteGenerator.fromImageProvider`) + `ensureContrast(bg, fg, minRatio:4.5)` + `contrastRatio`/`relativeLuminance`/`getTextColorForBackground`. T2/T4 위젯이 fallback이 아닌 실제 표지 추출 색으로 렌더되도록 통합. 그 후 PR9(에디터 MVP — controller·언두·영속화 + 실제 `quoteByIdProvider` 데이터) → PR10(렌더+공유, `RepaintBoundary.toImage` + 폰트 로드 보장 + `share_plus`/`path_provider`) → PR11(`cards` 테이블 + `design jsonb` + 공유 성공 시 비차단 INSERT) → PR12(마감 — 언두 ≥20·텍스트 ±·워터마크 토글·auto-fit 경고·접근성·골든). PR7 산출: `lib/features/card_editor/{domain,data,presentation/widgets}/*` — `sealed class CardTemplate` ×5(`MinimalTemplate`/`WarmTemplate`/`MonoTemplate`/`CoverExtractTemplate`/`TypographyTemplate`) + `supports/recommended/byId/all` + `QuoteCardData(quoteText, bookTitle, bookAuthor, bookPublisher, coverUrl)` + 5종 위젯(절대 1080×{1920/1080/1350} 캔버스, `FittedBox`로 화면 폭 축소 — "미리보기=export") + `QuoteCard` switch 디스패처(`fallbackFor` 헬퍼) + `CardWatermark` 공통 + `color_utils.lightenToBackground/toMidTone` + `getTypographyFontSize`/`splitIntoPoetryLines`(chunk별 강제 줄바꿈 보존). `card_editor_screen` = 비율 세그먼트(`SegmentedButton`) + 중앙 큰 미리보기(`AspectRatio`+`FittedBox`) + 하단 5종 미니카드 가로 한 줄 — mock 데이터로 시각 검증 가능. `share_plus`는 이미 pubspec, `path_provider`/`gal`은 PR10에서 추가. 테스트 19개 추가(template 게이트·recommended·byId·`splitIntoPoetryLines` 4종·`getTypographyFontSize`·5종 build smoke·3비율 분기).
