...
## ▶ 다음 세션 시작점 (2026-05-15 기준)

**상태**: Stage 0~1 완료 + 화면 설계(Stage 0b 연장) 완료 + **Stage 2 진행 중 — PR1·2·3·4·5·6 + 책 별점 완료. Stage 2 본 작업 종료, 다음은 Stage 3(카드)**. `flutter analyze` clean, `flutter test` 40개 통과. 마이그레이션 4개(`quotes`, `user_books.rating`, `my_quote_mood_counts`, +Stage1 4개) 원격 적용 완료. main에 push됨. 릴리스 APK 빌드해 실기기(SM F956N) 설치 진행.
...
**바로 이어서 할 것** → **Stage 3 (카드 — 가장 공들일 단계)**. 설계: `screens/card-editor.md` + `screens/card-share.md`. 순서: 5개 템플릿 위젯(`sealed class CardTemplate` ×5, 위젯 트리) → 색 추출(`palette_generator` → `ExtractedPalette` + `palette_service` LRU + `ensureContrast` AA 4.5:1) → 카드 편집기(3단 레이아웃, 언두 ≥20) → 이미지 export(`RepaintBoundary.toImage`, 폰트 로드 보장, `share_plus`/`path_provider`/`gal`) → `cards` 테이블(`design jsonb`, `on delete cascade auth.users`) → SNS 공유 시트(V1=`share_plus` OS 시트) → deep link 받기 "내 서재 담기 1탭"(handler·`/book/:id`는 PR6에서 이미 일반화됨 — `deep-link-receive.md §6` 잔여는 미로그인 자동담기 정도). 현행 `card_editor_screen`은 스텁.
