...
## ▶ 다음 세션 시작점 (2026-05-23 기준 — **책글귀 재리브랜드(복로그 충돌 회피). Play Console 표시명·스토어 자산 갱신 대기**)

**2026-05-23 산출 — 브랜드 재정렬**:

- **책글귀 재리브랜드** — Play Store에서 컨셉이 거의 동일한 "복로그" 앱(1만↓ 다운로드) 발견. 음운·시각 유사로 사칭·혼동 리스크 + 검색 누수 → 비공개 테스트 단계인 지금이 표시명을 바꿀 마지막 골든타임. "북로그" → "**책글귀**"로 재변경. 어원: 책 + 句(귀) = 책의 한 구절(글귀의 책 버전, "book quote"). 어제 `4a5d2c5` 변경 패턴을 동일하게 재치환 — Android `android:label`·iOS `CFBundleDisplayName`·인앱 문구·카드 워터마크(`tokens.dart`)·약관/개인정보/계정삭제 HTML·웹 메타데이터·tool 스크립트 docstring + 워드마크·신고·차단 마이그레이션 코멘트. **불변** (어제와 동일): 패키지 ID `io.github.tgparkk.bookquote`·Dart 패키지명·딥링크 스킴 — 코드 식별자는 처음부터 `bookquote`라 한국어 표시명(책귀=冊句, 책의 글귀)과 영문 식별자(book quote)가 다시 의미 일치.
- **네이밍 회의 기록(메모리)** — 매니저 모드 3라운드 후 책귀(원래 이름)와 책귀의 직설형 책글귀 비교 → 책귀의 약점("친구가 책귀가 뭐야?" 질문)을 책글귀가 정확히 보완(글귀=표준국어대사전 표제어, 즉시 이해). 회의 중 직역 매핑(DogEar) 실수 + V1.0 영문 ASO 평가 실수 2건 — `feedback_brand_translation.md` 메모리에 *어원 추정 금지·V1.0 단계 글로벌화 평가 금지* 기록.

**Play Console 영향 (2026-05-23 미완)**:
- 앱 표시명 "북로그" → "책글귀"로 콘솔에서 직접 변경
- 피처 그래픽 PNG 재생성·재업로드 (`python tool/generate_store_assets.py` — 워드마크 텍스트 변경 반영)
- 스크린샷 재캡처 — 홈 AppBar·로그인 환영 문구·me 화면 등에 표시명 노출됨

**▶ 다음 세션 할 일**:
1. **Play Console 표시명을 "책글귀"로 변경** — 우선 처리. 어제 입력한 스토어 등록정보도 표시명 일관성 위해 함께 검토.
2. **스토어 자산 재생성** — `python tool/generate_store_assets.py` 실행하여 피처 그래픽(워드마크 "책글귀") 재생성. 아이콘은 텍스트 무관이라 기존 이미지 유지.
3. **스크린샷 재캡처** — 표시명 변경 반영된 빌드 설치 후 4개 화면 재캡처 → `python tool/process_screenshots.py`로 정규화.
4. **AAB 재빌드 + 업로드** — `flutter build appbundle --release --dart-define-from-file=.env.json` (PR27 + 책글귀 반영). 업로드 후 Play 앱 서명 SHA-1 확보 → Google Cloud Console 등록.
5. Play Console 앱 카테고리·연락처 + 스토어 등록정보 입력 완료 → 11/11.
6. 데이터 보안 폼에 "비정상 종료 로그·진단" 추가 (Crashlytics 반영).
7. 비공개 테스트 트랙 — 테스터 12명·14일.
