...
- **[x] PR-PB** — `firebase_messaging` 통합. `lib/features/notifications/data/push_service.dart`(권한 요청·`register_device_token` RPC·onTokenRefresh 재등록·탭→`data.route` 라우팅·로그아웃 토큰 삭제) + main.dart(백그라운드 핸들러 등록, 로그인/로그아웃 `ref.listen`으로 start/stop, 콜드스타트 start) + `POST_NOTIFICATIONS` 권한. `flutter analyze` clean. ⚠️ 실발송은 PC 콘솔 셋업 필요.
- **[~] PR-PC** — Edge Function `supabase/functions/push-notification/index.ts`(웹훅 record → prefs 확인 → device_tokens → 서비스계정 RS256 JWT OAuth2 → FCM HTTP v1 발송 → stale 토큰 정리, actor 비공개 익명). **코드 완료, 미배포.** ⏳ 사람 손 필요: Firebase Cloud Messaging 활성화 + 서비스계정 키 → `FCM_SERVICE_ACCOUNT`·`WEBHOOK_SECRET` Edge secret + `functions deploy --no-verify-jwt` + notifications INSERT Database Webhook(헤더 `x-webhook-secret`). 상세 `docs/ops/fcm-push-setup.md`. 설정 토글 UI는 후속.
