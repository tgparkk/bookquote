...
- [ ] **PR16-A** 스키마 + 크립토 코어 — `supabase/migrations/20260517_quotes_e2ee.sql`(`text_encrypted bytea`·`manual_book_text_encrypted bytea`·`crypto_version smallint`·`is_private boolean default false` + CHECK 재정의 + partial index) + `20260518_user_crypto_envelopes.sql`(envelope 테이블 + RLS 본인만). `lib/core/crypto/{key_service,encryptor,envelope_service,crypto_version}.dart` — AES-256-GCM 래퍼 + `flutter_secure_storage` 캐시 + PBKDF2-HMAC-SHA512 600k. 단위 테스트: roundtrip / 잘못된 키 sentinel / KDF 벤치마크 SM F956N 실측. pubspec: `cryptography ^2.7.0`.
- [ ] **PR16-B** 도메인·repository wiring — `Quote.isPrivate` + `QuoteRepository`가 isPrivate면 자동 암복호화. `QuoteInput`에도 isPrivate. `QuoteOutbox`도 큐잉 직전 암호화(`shared_preferences`에 ciphertext만 남게). 키 부재 시 sentinel(빈 텍스트 + locked 플래그). `cards.design`에 quote text 사본이 들어가는지 grep 검증.
