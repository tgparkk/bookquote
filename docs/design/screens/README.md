...
> 이 문서들은 **구현 직전 명세**다. `lib/app/router.dart`의 실제 라우트, `lib/core/theme/tokens.dart`의 토큰, `docs/design/templates/`의 카드 명세, `docs/discovery/error-handling.md`의 에러 분류를 단일 진실로 참조한다. 모순이 보이면 코드/토큰 쪽이 우선 — 문서를 고친다.

---

## 화면 인벤토리 & 진행 그룹

| 그룹 | 화면 | 라우트 | 현재 상태 | 설계 문서 |
|---|---|---|---|---|
| **1** (Stage 2~3, 최우선) | 인용구 입력 | `/quote/new` | 스텁 | ✅ [`quote-input.md`](quote-input.md) |
| **1** | 인용구 목록·상세 | 서재 탭 내 "인용구" 뷰 (`/library?tab=quotes`) | 없음 | ✅ [`quote-list.md`](quote-list.md) |
| **1** | 카드 에디터 | `/quote/:id/card` | 스텁 | ✅ [`card-editor.md`](card-editor.md) |
| **1** | 카드 공유·저장 시트 | (모달, 카드 에디터에서) | 없음 | ✅ [`card-share.md`](card-share.md) |
| **1** | 받은 카드 → 책 담기 (deep link 진입) | `/book/:id?from=share` | 부분 (`/book/:id` read-only만) | ✅ [`deep-link-receive.md`](deep-link-receive.md) |
| **2** | 홈 (내 인용 피드) | `/` | 스텁 | ✅ [`home.md`](home.md) |
| **2** | Me (계정·설정·내보내기·탈퇴) | `/me` | 반쯤 | ✅ [`me.md`](me.md) |
| **2** | 책 상세 (보강) | `/book/:id` | ✅ read-only | ✅ [`book-detail.md`](book-detail.md) |
| **3** (역정리) | 스플래시 | `/splash` | ✅ | ✅ [`splash.md`](splash.md) |
| **3** | 로그인 | `/auth/login` | ✅ | ✅ [`login.md`](login.md) |
| **3** | 인증 콜백 | `/auth/callback` · `/callback` | ✅ | ✅ [`auth-callback.md`](auth-callback.md) |
| **3** | 서재 (보강) | `/library` | ✅ | ✅ [`library.md`](library.md) |
| **3** | 책 검색 시트 (보강) | (모달, 서재·인용입력에서) | ✅ | ✅ [`book-search-sheet.md`](book-search-sheet.md) |

> **모든 화면 설계 문서 1차 작성 완료** (2026-05-12). 그룹 2·3 화면의 시각 목업(`screens.html` 확장)은 다음 단계. 그룹 3 역정리 문서들은 "현행 동작"(코드 기준) + "수정·보강 권고"를 §7에 분리해 담았다 — 코드 레벨 버그 목록은 각 문서 §7 + `docs/sessions/2026-05-12-screen-design-b.md`.
...
```
/splash ──(로그인됨)──▶ ┌─────────── StatefulShellRoute (BottomNav 4슬롯) ───────────┐
        │               │  [0] /           홈 — 내 인용 피드 + 받은 카드 함          │
        └─(미로그인)─┐   │  [1] /library    서재 — 책 리스트(+구절수 배지·표지색 띠)    │
                     │   │  [2] (＋ sentinel) ──push──▶ /quote/new  (풀스크린, 셸 밖)  │
   /auth/login ◀─────┤   │  [3] /me         Me — 계정·내보내기·약관·탈퇴               │
        │            │   └────────────────────────────────────────────────────────────┘
   /auth/callback    │
   /callback         │   풀스크린 (rootNavigatorKey, 셸 밖):
                     │     /quote/new[?bookId=]   인용구 입력 (직접 입력 / 클립보드 붙여넣기)
   게스트 허용 ◀─────┘        └─ "카드 만들기" ──push──▶ /quote/:id/card  카드 에디터
   /book/:id[?from=]                                        └─ "공유" ──▶ 공유·저장 시트(모달)
        ▲                                                                  └─ 카카오톡 → 받는 사람
        └── 받는 사람 deep link: io.github.tgparkk.bookquote://book/:id?from=share
              ├─ 로그인됨   → 책 상세 + "내 서재 담기" 1탭
              └─ 미로그인   → 책 상세 read-only → "담기" → /auth/login → 복귀해서 담기

  모달 시트(어느 셸 탭에서도): showBookSearchSheet  (알라딘 검색 + 캐시 + ISBN 직접 등록)
```

탭 구조·풀스크린 규칙·게스트 라우트는 `DECISIONS.md 2026-05-10 (go_router 구조)` 와 `lib/app/router.dart` 가 진실. **홈 탭의 정의는 "친구 새 인용"(코드 현행 placeholder) → "내 인용 피드 + 받은 카드 함"으로 변경** (DECISIONS 갱신 예정, `competitor-screen-analysis §5.5`). follow 타임라인은 V1.5에 같은 피드에 합친다.
